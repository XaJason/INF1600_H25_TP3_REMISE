...
std::string tests[] = {
    "constructor, realPart(), imagPart(), modulus(), operator+, operator*, "
...
int complexConstructorTest() {
    ...
    std::cout << std::setw(12) << std::left << "result: " << result
              << std::endl;
    std::cout << std::setw(12) << std::left << "expected:  " << expected
              << std::endl;
    if (result.real != a || result.imag != b) {
        std::cout
            << ansiColorRed
            << "Les attributs de la classe ne sont pas initialises adequatement"
            << std::endl;
    }
    //     std::cout << ansiColorRed << "Attribut -real- n'est pas initialise
    //     adequatement" << std::endl;
    // if (result.imag != b)
    // if (result.real == a && result.imag == b){
    else {
...
int complexRealPartTest() {
    ...
    if (!areFloatsEqual(result, expected))
        std::cout << ansiColorRed
                  << "realPart() n'obtient pas adequatement la partie reelle "
                     "du nombre complexe"
                  << std::endl;
    else {
        points = testValues[1];
...
int complexImagPartTest() {
    ...
    if (!areFloatsEqual(result, expected))
        std::cout << ansiColorRed
                  << "imagPart() n'obtient pas adequatement la partie reelle "
                     "du nombre complexe"
                  << std::endl;
    else {
...
int complexOperationPlTest() {
    ...
    std::cout << std::setw(12) << std::left << "expected:  " << expected
              << std::endl;
    std::cout << ansiColorRed;
    if (!areFloatsEqual(result.real, expected.real()) ||
        !areFloatsEqual(result.imag, expected.imag()))
        std::cout << ansiColorRed
                  << "La surcharge de operateur+ entre 2 Complex ne fonctionne "
                     "pas adequatement"
                  << std::endl;
    else {
...
int complexModulusTest() {
    ...
    if (!areFloatsEqual(result, expected))
        std::cout
            << ansiColorRed
            << "Le calcul du module du Complex ne fonctionne pas adequatement"
            << std::endl;
    else {
...
int mandelbrotRecurseTest() {
    ...
    std::cout << ansiColorYellow;
    std::cout << std::setw(20) << std::left << "result   (0, 0):" << result0
              << std::endl;
    std::cout << std::setw(20) << std::left << "expected (0, 0): " << expected0
              << std::endl;
    std::cout << std::setw(10) << std::left << "result   (" << c << "," << d
              << "): " << resultKnown1 << std::endl;
    std::cout << std::setw(10) << std::left << "expected (" << c << "," << d
              << "): " << expectedKnown1 << std::endl;
    std::cout << std::setw(10) << std::left << "result   (" << e << "," << f
              << "): " << resultKnown2 << std::endl;
    std::cout << std::setw(10) << std::left << "expected (" << e << "," << f
              << "): " << expectedKnown2 << std::endl;
    std::cout << std::setw(10) << std::left << "result   (" << g << "," << h
              << "): " << resultKnown3 << std::endl;
    std::cout << std::setw(10) << std::left << "expected (" << g << "," << h
              << "): " << expectedKnown3 << std::endl;
    std::cout << std::setw(20) << std::left << "result   (3, 4):" << resultBig
              << std::endl;
    std::cout << std::setw(20) << std::left
              << "expected (3, 4): " << expectedBig << std::endl;
    std::cout << "result -- random --  (" << a << "," << b
              << "): " << resultRandom << std::endl;
    std::cout << "expected --random -- (" << a << "," << b
              << "): " << expectedRandom << std::endl;
    std::cout << ansiColorRed;
    if (result0 != expected0 || resultRandom != expectedRandom ||
        resultKnown1 != expectedKnown1 || resultKnown2 != expectedKnown2 ||
        resultKnown3 != expectedKnown3 || resultBig != expectedBig)
        std::cout << ansiColorRed
                  << "La fonction recursive de Mandelbrot ne fonctionne pas"
                  << std::endl;
    else {
        std::cout << ansiColorGreen << "Test Mandelbrot recursif OK!"
                  << std::endl;
        points = testValues[6];
    }
    std::cout << ansiColorReset << std::endl;
